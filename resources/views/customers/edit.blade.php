            ...
            <div class="card-body">
                ...
                <form method="POST" action="{{ route('customers.update', $customer) }}" id="form-edit">
                    ...
                    <div class="mb-3">
                        ...
                        <input type="tel" class="form-control @error('phone') is-invalid @enderror" id="phone" name="phone" value="{{ old('phone', $customer->phone) }}" pattern="[0-9]{11,15}" minlength="11" maxlength="15" title="Kisaran No.HP 11-15 digit" oninput="this.value = this.value.replace(/[^0-9]/g, '')" placeholder="08XXXXXXXXXX" required>
                        @error('phone')
                        <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
